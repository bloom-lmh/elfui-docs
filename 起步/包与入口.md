...
ElfUI 把主线能力和扩展能力拆开。新项目从 `elfui` 开始，链式组件只在需要时使用 `@elfui/chain`。

## 入口选择

| 场景               | 安装                           | 导入                                               |
| ------------------ | ------------------------------ | -------------------------------------------------- |
| 新项目 / 组件库    | `elfui` + `@elfui/vite-plugin` | `import { defineHtml, createApp } from "elfui"`    |
...
| 包                   | 职责                                             |
| -------------------- | ------------------------------------------------ |
| `elfui`              | 宏组件与根挂载入口，聚合常用响应式和 runtime API |
...
| 入口                    |     gzip | 说明                                    |
| ----------------------- | -------: | --------------------------------------- |
| `elfui`                 | 10.52 KB | 主入口，不带 runtime compiler           |
| `@elfui/chain`          | 21.19 KB | 链式 `.template()`，带 runtime compiler |
| `elfui + @elfui/router` | 15.00 KB | 主入口加路由                            |
| `@elfui/reactivity`     |  4.17 KB | 只用响应式                              |

## 边界

`elfui` 不导出 `createComponent()`、`ElementBuilder`、`compile()` 或运行时字符串模板能力。需要链式 API 时，去“生态 / Chain 链式组件”。
